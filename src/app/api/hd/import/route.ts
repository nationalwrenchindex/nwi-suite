...
const UNIT_FIELDS = [
  ...
  { key: 'fleet_name',     label: 'Fleet Account Name', required: false },
]
...
export async function POST(req: NextRequest) {
  ...
  const prompt = `You are mapping CSV columns to database fields for a fleet management import.
...
{ "csv_header": "exact header from CSV", "field_key": "target field key or null if no match", "confidence": "high|medium|low" }

Include all CSV headers in your response. Use null for field_key when no field matches. Return only valid JSON, no explanation.`
