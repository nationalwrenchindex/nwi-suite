...
export default async function NewDOTInspectionPage({
  ...
  const [
    ...
    supabase
      .from('profiles')
      .select('hd_tech_name, hd_epa_cert_number')
      .eq('id', user.id)
      .single(),
  ])
  ...
  return (
    <main className="flex-1 p-6">
      <DOTInspectionForm
        ...
        profile={{
          hd_tech_name:        (profile as { hd_tech_name?: string | null } | null)?.hd_tech_name        ?? null,
          hd_epa_cert_number:  (profile as { hd_epa_cert_number?: string | null } | null)?.hd_epa_cert_number ?? null,
        }}
