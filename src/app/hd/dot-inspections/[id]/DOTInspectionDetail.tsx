...
export default function DOTInspectionDetail({
  inspection,
  branding,
}: {
  inspection: DOTInspection
  branding:   Branding
}) {
    ...
    <>
      ...
      <style dangerouslySetInnerHTML={{ __html: `
        @media print {
          ...
          /* Brand logo and signature keep their own colours on paper. */
          #print-root img { display: inline-block !important; -webkit-print-color-adjust: exact; print-color-adjust: exact; }
        }
      ...
      <div className="no-print max-w-3xl mx-auto space-y-5 p-4 sm:p-6">
        ...
        <div className="rounded-xl p-4" style={{ background: '#0d1820', border: '1px solid #1e3040' }}>
          ...
          <BrandFooter className="mt-2 text-white" />
        </div>
      ...
      <div id="print-root" className="print-show" style={{ fontFamily: 'Arial, sans-serif', color: '#111', lineHeight: 1.4 }}>
        {/* Print header — the subscriber's brand, not NWI's. The logo comes from
            resolveBranding, which prefers the uploaded business_logo_url and
            falls back to the hd_company_logo_url the DOT form has always read,
            so a subscriber who only pasted a URL keeps the same header. Name in
            text whenever no logo resolves. */}
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', borderBottom: '3px solid #E85D24', paddingBottom: 12, marginBottom: 16 }}>
          <div>
            ...
            <div style={{ fontSize: 18, fontWeight: 700, letterSpacing: 1, textTransform: 'uppercase', marginTop: 6 }}>
        ...
        {/* Footer — the trademark attribution stays on the printed record even
            when the header is fully white-labelled. */}
        <div style={{ borderTop: '1px solid #ccc', paddingTop: 6, fontSize: 8, color: '#999' }}>
          <div style={{ display: 'flex', justifyContent: 'space-between' }}>
            <span>Inspection ID: {inspId} · This record was electronically signed and is locked.</span>
            <span>Generated {new Date(inspection.created_at).toLocaleString()}</span>
          </div>
          <div style={{ marginTop: 3 }}>{NWI_TRADEMARK_FOOTER}</div>
        </div>
      </div>
    </>
  )
