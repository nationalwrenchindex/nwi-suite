...
export default async function HDDashboardPage({
  ...
  return (
    <main className="flex-1 p-4 sm:p-6 space-y-6">
      ...
      >
        <div className="absolute -right-8 -top-8 w-40 h-40 rounded-full pointer-events-none" style={{ background: 'rgba(var(--hd-ink-rgb), 0.04)' }} />
        <div className="absolute right-10 bottom-0 w-16 h-16 rounded-full pointer-events-none" style={{ background: `${HD_ORANGE}25` }} />
        <div className="relative z-10">
          <p className="text-xs uppercase tracking-widest mb-1" style={{ color: 'rgba(var(--hd-ink-rgb), 0.5)' }}>
            HD Suite · {dayLabel}
          </p>
          <h1 className="font-condensed font-bold text-3xl text-white tracking-wide">
            {profile?.full_name ?? user.email}
          </h1>
          <p className="text-sm mt-0.5" style={{ color: 'rgba(var(--hd-ink-rgb), 0.6)' }}>{profile?.business_name ?? 'HD Suite'}</p>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        {/* Open Work Orders */}
        <div className="lg:col-span-2 rounded-xl p-5" style={{ background: 'var(--hd-card)', border: '1px solid var(--hd-border)' }}>
          ...
          {!workOrders || workOrders.length === 0 ? (
            <div className="py-10 text-center">
              <p className="text-sm" style={{ color: 'rgba(var(--hd-ink-rgb), 0.25)' }}>No open work orders</p>
              <Link href="/hd/work-orders"
              ...
              {workOrders.map(wo => (
                <div key={wo.id} className="py-3 flex items-center gap-3">
                  <div className="flex-1 min-w-0">
                    ...
                    <p className="text-xs" style={{ color: 'rgba(var(--hd-ink-rgb), 0.4)' }}>{wo.service_type ?? 'Service'}</p>
                  </div>
                  <span
                    className="text-xs px-2 py-0.5 rounded-full font-medium"
                    style={{
                      background: wo.status === 'in_progress' ? `${HD_ORANGE}25` : 'rgba(var(--hd-ink-rgb), 0.08)',
                      color:      wo.status === 'in_progress' ? HD_ORANGE : 'rgba(var(--hd-ink-rgb), 0.5)',
                    }}
                  >
        ...
        <div className="rounded-xl p-5" style={{ background: 'var(--hd-card)', border: '1px solid var(--hd-border)' }}>
          ...
          <div className="mb-3 p-3 rounded-lg" style={{ background: 'var(--hd-inner)' }}>
            <p className="text-xs uppercase tracking-widest mb-1" style={{ color: 'rgba(var(--hd-ink-rgb), 0.4)' }}>Total Refrigerant Tracked</p>
            <p className="font-condensed font-bold text-2xl text-white">{totalEpaLbs.toFixed(1)} <span className="text-sm font-normal" style={{ color: 'rgba(var(--hd-ink-rgb), 0.4)' }}>lbs</span></p>
          </div>
          {!epaLog || epaLog.length === 0 ? (
            <p className="text-xs text-center py-6" style={{ color: 'rgba(var(--hd-ink-rgb), 0.25)' }}>No EPA log entries</p>
          ) : (
            <div className="space-y-2">
              {epaLog.slice(0, 4).map(e => (
                <div key={e.id} className="flex items-center gap-2 text-xs">
                  <div className="flex-1 truncate" style={{ color: 'rgba(var(--hd-ink-rgb), 0.7)' }}>
      ...
      <div className="rounded-xl p-5" style={{ background: 'var(--hd-card)', border: '1px solid var(--hd-border)' }}>
        ...
        {pmDueSoon.length === 0 ? (
          <div className="py-8 text-center">
            <p className="text-sm" style={{ color: 'rgba(var(--hd-ink-rgb), 0.25)' }}>
      ...
      <div>
        <p className="text-xs uppercase tracking-widest mb-3" style={{ color: 'rgba(var(--hd-ink-rgb), 0.35)' }}>Quick Access</p>
        <div className="grid grid-cols-2 lg:grid-cols-4 gap-3">
          {[
            ...
            >
              ...
              <p className="text-xs" style={{ color: 'rgba(var(--hd-ink-rgb), 0.4)' }}>{sub}</p>
            </Link>
          ))}
        </div>
      </div>
