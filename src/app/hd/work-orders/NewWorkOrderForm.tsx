...
export default function NewWorkOrderForm({ units, fleetAccounts, presetAccountId }: { units: Unit[]; fleetAccounts: FleetAccount[]; presetAccountId?: string | null }) {
  ...
  return (
    <form onSubmit={submit} className="rounded-xl p-6 mb-6 space-y-4" style={{ background: 'var(--hd-card)', border: `1px solid ${HD_ORANGE}50` }}>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
        ...
        <div style={{ position: 'relative' }}>
          <label className={lblCls} style={{ color: 'rgba(var(--hd-ink-rgb), 0.4)' }}>Customer *</label>
          <input
            value={customerName}
            ...
            <div style={{ position: 'absolute', zIndex: 20, left: 0, right: 0, marginTop: 4, background: 'var(--hd-card)', border: '1px solid var(--hd-border)', borderRadius: 8, overflow: 'hidden', maxHeight: 220, overflowY: 'auto' }}>
              {acctMatches.map(a => (
                <button key={a.id} type="button" onMouseDown={ev => { ev.preventDefault(); pickAccount(a) }}
                  className="w-full text-left px-3 py-2.5 text-sm text-white hover:bg-white/5" style={{ borderBottom: '1px solid var(--hd-border)' }}>
                  {a.fleet_name}
                </button>
              ))}
            </div>
        ...
        <div>
          <label className={lblCls} style={{ color: 'rgba(var(--hd-ink-rgb), 0.4)' }}>Unit</label>
        ...
        <div>
          <label className={lblCls} style={{ color: 'rgba(var(--hd-ink-rgb), 0.4)' }}>Job Type</label>
        ...
        <div>
          <label className={lblCls} style={{ color: 'rgba(var(--hd-ink-rgb), 0.4)' }}>Estimated Hours</label>
        ...
        <div>
          <label className={lblCls} style={{ color: 'rgba(var(--hd-ink-rgb), 0.4)' }}>Scheduled Date *</label>
          <input type="date" value={scheduledDate} onChange={e => setScheduledDate(e.target.value)} className={inputCls} style={inputStyle} />
        </div>

        <div>
          <label className={lblCls} style={{ color: 'rgba(var(--hd-ink-rgb), 0.4)' }}>Scheduled Time</label>
      ...
      <div>
        <label className={lblCls} style={{ color: 'rgba(var(--hd-ink-rgb), 0.4)' }}>Service Address</label>
        <div className="rounded-lg p-3" style={{ background: '#F4F5F7', border: '1px solid var(--hd-border)' }}>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
        <div>
          <label className={lblCls} style={{ color: 'rgba(var(--hd-ink-rgb), 0.4)' }}>Address Line 1</label>
        ...
        <div>
          <label className={lblCls} style={{ color: 'rgba(var(--hd-ink-rgb), 0.4)' }}>Address Line 2</label>
      ...
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-4">
        <div className="col-span-2">
          <label className={lblCls} style={{ color: 'rgba(var(--hd-ink-rgb), 0.4)' }}>City</label>
        ...
        <div>
          <label className={lblCls} style={{ color: 'rgba(var(--hd-ink-rgb), 0.4)' }}>State</label>
        ...
        <div>
          <label className={lblCls} style={{ color: 'rgba(var(--hd-ink-rgb), 0.4)' }}>Zip</label>
      ...
      <div>
        <label className={lblCls} style={{ color: 'rgba(var(--hd-ink-rgb), 0.4)' }}>Job Description</label>
      ...
      <div>
        <label className={lblCls} style={{ color: 'rgba(var(--hd-ink-rgb), 0.4)' }}>Notes</label>
      ...
      <div className="flex gap-3 pt-1">
        ...
        <Link href="/hd/work-orders" className="px-4 py-2.5 rounded-lg text-sm border" style={{ color: 'rgba(var(--hd-ink-rgb), 0.5)', borderColor: 'var(--hd-border)' }}>Cancel</Link>
      </div>
    </form>
  )
}
