...
// Signed aerial inspection record — screen view and print document in one.
//
// Item 7 ("generate a downloadable signed PDF") is served the same way the DOT
// inspection already does it: a print stylesheet plus window.print(), which the
// browser turns into a PDF. That keeps both inspection families producing
// identical-looking documents and avoids adding a PDF rendering dependency to
// produce something the platform already does well.
      ...
      <div className="no-print flex items-center justify-between mb-4">
        <div>
          ...
          <p className="text-white/40 text-xs mt-0.5">
            {record.inspection_id ?? record.id.slice(0, 8)} · {AERIAL_TYPE_LABEL[record.inspection_type]}
          </p>
        </div>
        <button
          type="button"
