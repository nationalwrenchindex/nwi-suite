...
// Shared rendering + submit engine for all three aerial cadences.
//
// The cadences differ only in their AerialFormDef, so there is one component
// rather than three near-identical forms. Visual language is lifted from
// DOTInspectionForm so the two inspection families look and print alike.
...
export default function AerialChecklist({
  ...
  const [data,        setData]        = useState<AerialInspectionData>(() => emptyData(def))
      ...
      sections: {
        ...
        [sectionId]: {
          items: {
            ...
            [itemId]: {
              ...(prev.sections[sectionId]?.items[itemId] ?? { result: '', notes: '' }),
  ...
  async function submit() {
    ...
    setError(null)
    try {
      const res = await fetch('/api/hd/aerial-inspections', {
        method:  'POST',
  ...
  return (
    <div className="space-y-5">
      ...
      <div className="rounded-xl p-4" style={{ background: '#111920', border: '1px solid #1e3040' }}>
        <p className="text-white font-condensed font-bold text-lg tracking-wide">{def.title}</p>
      ...
      {/* Critical banner — OSHA out-of-service determination */}
      {critical && (
