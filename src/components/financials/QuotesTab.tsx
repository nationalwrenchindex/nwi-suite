...
function QuoteDetailModal({
  ...
  // Live status for polling-driven updates
  ...
  // ── Poll for customer approval every 10s when quote is sent ───────────────
  useEffect(() => {
    if (liveStatus !== 'sent') return
    const ctrl = new AbortController()
    const id = setInterval(async () => {
      try {
        const res = await fetch(`/api/quotes/${initialQuote.id}`, { signal: ctrl.signal })
        if (!res.ok) return
        const json = await res.json()
  ...
  const timeline: { label: string; ts: string | null; detail?: string }[] = [
    ...
    { label: 'Sent',      ts: liveQuote.sent_at,
      detail: liveQuote.times_sent && liveQuote.times_sent > 1
        ? `(sent ${liveQuote.times_sent}x)` : undefined },
    { label: 'Viewed',    ts: liveQuote.viewed_at,
      detail: liveQuote.view_count > 0
        ? `(${liveQuote.view_count} view${liveQuote.view_count !== 1 ? 's' : ''})` : undefined },
    { label: 'Approved',  ts: liveQuote.approved_at },
    { label: 'Declined',  ts: liveQuote.declined_at },
    { label: 'Converted', ts: liveQuote.converted_at },
  ].filter(e => e.ts)
      ...
      >
        <div className="relative w-full max-w-2xl bg-[#1a1a1a] border border-white/10 rounded-2xl shadow-2xl my-8">
          ...
          <div className="px-6 py-5 space-y-6">
            ...
            {liveStatus === 'declined' && liveQuote.customer_response_note && (
              <div className="space-y-1">
                <p className="text-white/30 text-xs uppercase tracking-widest">Customer Note</p>
                <p className="text-white/60 text-sm italic">&ldquo;{liveQuote.customer_response_note}&rdquo;</p>
              </div>
            )}
